...
def fourier_coeffs(n, k_max):
    ...
    for k in range(k_max + 1):
        ...
        integral, _ = scipy.integrate.quad(integrand, -math_pi, math_pi)
        a_k = 1 / math_pi * integral
        a_coeffs.append(a_k)
        if k > 0:
            integrand = lambda x: f(x, n) * np.sin(k * x)
            integral, _ = scipy.integrate.quad(integrand, -math_pi, math_pi)
            b_k = 1 / math_pi * integral
            b_coeffs.append(b_k)
...
print("\nЗначення ряду Фур'є, при n=18: ", f(n, k_max), "\n")
...
# побудова графіку
x_vals = np.linspace(-np_pi, np_pi , 500)
...
def save_results_to_files(a_coeffs, b_coeffs, n, k_max, rel_error):
    ...
    with open("relative_error.txt", "w") as f:
        f.write(f"Похибка:\t{rel_error:.8f}")
...
plt.plot(k_vals_freq, y_vals_freq, label=f'Ряд фур\'є, N={k_max}', color="red")
plt.xlabel('k')
plt.ylabel('y')
plt.legend()
plt.show()
...
plt.plot(x_vals, y_series_vals, label=f'Ряд фур\'є, N={k_max}', color="green")
